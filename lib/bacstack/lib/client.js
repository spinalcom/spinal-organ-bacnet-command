...
class Client extends EventEmitter {
  ...
  _processUnconfirmedServiceRequest(address, type, service, buffer, offset, length) {
    ...
    if (service === baEnum.UnconfirmedServiceChoice.I_AM) {
       ...
       * });
       */
      this.emit('iAm', {address: address, deviceId: result.deviceId, maxApdu: result.maxApdu, segmentation: result.segmentation, vendorId: result.vendorId, SADR: this.source});
  ...
  whoIs(options) {
    ...
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, settings.dest /*this._settings.address*/, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
   ...
   * });
   */
  readProperty(address, destination, objectId, propertyId, options, next) {
    next = next || options;
    const settings = {
    ...
    const addr = (objectId.type === baEnum.ObjectTypesSupported.ANALOG_INPUT ||
      objectId.type === baEnum.ObjectTypesSupported.ANALOG_OUTPUT ||
      objectId.type === baEnum.ObjectTypesSupported.ANALOG_VALUE ||
      objectId.type === baEnum.ObjectTypesSupported.BINARY_INPUT ||
      objectId.type === baEnum.ObjectTypesSupported.BINARY_OUTPUT ||
      objectId.type === baEnum.ObjectTypesSupported.BINARY_VALUE ||
      objectId.type === baEnum.ObjectTypesSupported.DEVICE ||
      objectId.type === baEnum.ObjectTypesSupported.MULTI_STATE_INPUT ||
      objectId.type === baEnum.ObjectTypesSupported.MULTI_STATE_OUTPUT ||
      objectId.type === baEnum.ObjectTypesSupported.MULTI_STATE_VALUE ||
      objectId.type === 12 ||
      objectId.type === 70
    ) ? destination : address;
    ...
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE | baEnum.NpduControlBits.EXPECTING_REPLY, addr, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
   ...
   * });
   */
  writeProperty(address, destination, objectId, propertyId, values, options, next) {
    next = next || options;
    const settings = {
      maxSegments: options.maxSegments || baEnum.MaxSegmentsAccepted.SEGMENTS_65,
      maxApdu: options.maxApdu || baEnum.MaxApduLengthAccepted.OCTETS_1476,
    ...
    const addr = (objectId.type === baEnum.ObjectTypesSupported.ANALOG_OUTPUT ||
      objectId.type === baEnum.ObjectTypesSupported.ANALOG_VALUE ||
      objectId.type === baEnum.ObjectTypesSupported.BINARY_OUTPUT ||
      objectId.type === baEnum.ObjectTypesSupported.BINARY_VALUE ||
      objectId.type === baEnum.ObjectTypesSupported.MULTI_STATE_OUTPUT ||
   ...
   * });
   */
  readPropertyMultiple(address, destination, propertiesArray, options, next) {
    next = next || options;
    const settings = {
      maxSegments: options.maxSegments || baEnum.MaxSegmentsAccepted.SEGMENTS_65,
      maxApdu: options.maxApdu || baEnum.MaxApduLengthAccepted.OCTETS_1476,
    ...
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE | baEnum.NpduControlBits.EXPECTING_REPLY, destination/*address*/, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
    const type = baEnum.PduTypes.CONFIRMED_REQUEST | (settings.maxSegments !== baEnum.MaxSegmentsAccepted.SEGMENTS_0 ? baEnum.PduConReqBits.SEGMENTED_RESPONSE_ACCEPTED : 0);
