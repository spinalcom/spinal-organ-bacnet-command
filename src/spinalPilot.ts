...
class SpinalPilot {
	...
	public async sendPilotRequest(request: IRequest & { priority?: number }, endpointElement: SpinalBmsEndpoint): Promise<boolean> {
		try {
			...
			if (!request.priority) request.priority = this._getBacnetPriority();
	...
	private _getBacnetPriority(): number {
